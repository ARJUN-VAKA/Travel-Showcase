    ...
    >
      <div className={`flex flex-col ${isEven ? "md:flex-row" : "md:flex-row-reverse"} gap-6 md:gap-12 items-center w-full max-w-5xl mx-auto`}>
        ...
        <div className={`relative w-full ${item.orientation === "portrait" ? "md:w-1/3" : "md:w-2/3"} border-4 border-black shadow-md bg-white p-2 md:p-4 rotate-1`}>
          <div className="absolute -top-4 left-1/2 w-16 h-8 bg-[#ddd] opacity-80 rotate-3 z-10 mix-blend-multiply" />
          ...
          <div className="border-2 border-black overflow-hidden relative group">
            {item.type === "video" ? (
              <div className="relative aspect-video bg-black">
                ...
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none group-hover:hidden">
                  <div className="w-16 h-16 bg-primary border-4 border-black rounded-full flex items-center justify-center shadow-sm">
                    <div className="w-0 h-0 border-t-8 border-b-8 border-l-[12px] border-t-transparent border-b-transparent border-l-black ml-2" />
                  </div>
                </div>
              </div>
            ) : item.src ? (
              <img
                src={item.src}
                alt={item.caption}
                className={`w-full ${item.orientation === "portrait" ? "aspect-[3/4]" : "aspect-[16/9]"} object-cover grayscale-[10%] group-hover:grayscale-0 transition-all`}
              />
            ) : (
              <div className={`w-full ${item.orientation === "portrait" ? "aspect-[3/4]" : "aspect-[16/9]"} bg-muted flex items-center justify-center`}>
                <span className="font-black uppercase text-muted-foreground">No Media</span>
              </div>
            )}
          </div>
        </div>
...
export default function TripStory() {
  ...
  if (!trip) {
    return (
      <div className="min-h-screen flex items-center justify-center gap-4 bg-background flex-wrap p-6">
        <div className="border-4 border-black p-8 bg-destructive text-white font-black text-4xl shadow-md uppercase">
          Trip Not Found
        </div>
        <Link href="/" className="border-4 border-black px-6 py-3 font-bold bg-white text-black shadow-sm hover:shadow-md hover:-translate-y-1 transition-all uppercase">
          Go Back
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-background w-full pb-32 overflow-x-hidden">
      ...
      <div className="fixed top-6 left-6 z-50 flex gap-2">
        <Link
          href="/"
          className="inline-block border-4 border-black px-4 py-2 font-bold bg-primary text-black shadow-sm hover:shadow-md hover:-translate-y-1 transition-all uppercase"
        ...
        <Link
          href="/dashboard"
          className="inline-block border-4 border-black px-4 py-2 font-bold bg-white text-black shadow-sm hover:shadow-md hover:-translate-y-1 transition-all uppercase"
      ...
      <header className="min-h-[70vh] flex flex-col items-center justify-center relative border-b-8 border-black p-6" style={{ backgroundColor: trip.color }}>
        ...
        >
          <h1
            className="text-7xl md:text-9xl font-black text-white uppercase drop-shadow-[6px_6px_0_rgba(0,0,0,1)]"
          ...
          <div className="mt-4 flex items-center justify-center gap-4 flex-wrap">
            <span className="bg-white text-black border-4 border-black px-4 py-1 font-bold uppercase text-lg">{trip.date}</span>
            <span className="bg-white text-black border-4 border-black px-4 py-1 font-bold uppercase text-lg">{trip.friendCount} Friends</span>
          </div>
        </motion.div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 mt-32 relative">
        ...
        {trip.media.length > 0 ? (
          ...
          <div className="text-center border-4 border-black border-dashed p-20 bg-muted">
            <h2 className="text-4xl font-black uppercase text-muted-foreground">No memories yet.</h2>
            <p className="font-mono text-muted-foreground mt-2">Add photos and videos in the Dashboard.</p>
            <Link
              href="/dashboard"
              className="mt-6 inline-block border-4 border-black px-6 py-3 font-bold bg-primary text-black hover:bg-black hover:text-white transition-colors uppercase"
